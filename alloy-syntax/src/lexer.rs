...
pub enum TokKind {
    ...
    /// A backtick interpolated string with no hole.
    InterpStr,
    /// The opening of an interpolated string, from the backtick through the
    /// first `{`. Expression tokens follow.
    InterpHead,
    /// The text between two holes, from `}` through the next `{`.
    InterpMid,
    /// The text after the last hole, from `}` through the closing backtick.
    InterpTail,
    Number,
    LParen,
    RParen,
...
/// Scans the text of an interpolated string from `from` to its next hole
/// or its end.
fn interp_segment(b: &[u8], from: usize) -> Segment {
    let mut i = from;
...
mod tests {
    ...
    fn interp_strings_with_braces() {
        ...
        assert_eq!(toks[4].text(src), "} b`");
    }

    #[test]
    fn interp_strings_nest_and_hold_tables() {
        let src = "`x {t[{a = `in {y}`}]} z {q}`";
        let toks = lex(src).unwrap().toks;
        let kinds: Vec<TokKind> = toks.iter().map(|t| t.kind).collect();

        assert_eq!(kinds[0], TokKind::InterpHead);
        assert_eq!(kinds[kinds.len() - 1], TokKind::InterpTail);
        assert_eq!(
            kinds.iter().filter(|k| **k == TokKind::InterpMid).count(),
            1
        );
        assert_eq!(
            kinds.iter().filter(|k| **k == TokKind::InterpHead).count(),
            2
        );
        assert_eq!(
    ...
    #[test]
    fn plain_interp_string_is_one_token() {
        let toks = lex("`no holes` x").unwrap().toks;
