...
fn nested_blocks(stmt: &Stmt) -> Vec<&Block> {
    match stmt {
        ...
        Stmt::Match(m) => {
            let mut v: Vec<&Block> = m.arms.iter().map(|a| &a.block).collect();
        ...
        Stmt::Impl(i) => i.methods.iter().map(|f| &f.body.block).collect(),

        Stmt::PatternLocal(p) => p.else_block.iter().collect(),

        Stmt::Trait(t) => t
            .methods
            .iter()
            .filter_map(|m| m.body.as_ref().map(|b| &b.block))
            .collect(),
