...
compiles that .aly, .d.aly, or .alx file to stdout, or under --out. A
file that reports an error is not written; its last output stays. The
build runs no lint; alloy check reports them, and a denied one fails
that run.
