...
impl Doc {
    ...
    /// Whether a shadow range holds the author's text as written: no
    /// generated byte, and the same bytes where it maps in the source.
    /// An edit the child computes over such a range writes back only
    /// what the author wrote.
    pub fn copies_source(&self, start: (u32, u32), end: (u32, u32)) -> bool {
        let (Some(s), Some(e)) = (
            offset_of(&self.shadow, start.0, start.1),
            offset_of(&self.shadow, end.0, end.1),
        ) else {
            return false;
        };

        // An insertion writes before the byte at its position.
        if (s..e.max(s + 1)).any(|o| self.generated_offset(o)) {
            return false;
        }

        let (line, character) = self.to_source(start.0, start.1);

        offset_of(&self.source, line, character)
            .and_then(|at| self.source.get(at..at + e.saturating_sub(s)))
            .is_some_and(|text| self.shadow.get(s..e) == Some(text))
    }
