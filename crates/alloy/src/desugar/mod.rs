...
impl<'s> Desugar<'s> {
    ...
    fn scan_hoisted(&mut self, block: &Block) {
        ...
        // The declarations of one name, in order: a read below one of
        // them is that one's, so the next scan starts there.
        ...
        for s in &block.stmts {
            ...
            declared.push((name, decl.end as usize));
            ...
            for k in from..decl.start as usize {
                if quiet
                    .iter()
