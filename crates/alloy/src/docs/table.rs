...
pub const TABLE: &[(&str, &str)] = &[
    ...
    (
        "derive:Serialize",
        "```alloy\n@derive(Serialize)\n```\nGenerates `to_table`, `from_table`, and `serialize`, the plain-table forms for storage and remotes. `serialize` calls `to_table`, so the struct meets a `T: Serialize` bound.\n\nA field whose type is a struct of the file that also derives `Serialize`, or that type with `?`, goes through that struct's own pair, so the table nests and `from_table` gives the value back with its metatable. Two fields under one key, and a field named like a method the derive writes, report.",
    ),
    // Attributes
    (
        "@derive",
